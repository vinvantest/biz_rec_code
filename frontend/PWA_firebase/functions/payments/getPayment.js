...
//https://us-central1-bizrec-dev.cloudfunctions.net/createpaymentFunction?uid=HJIOFS#53345DD
//no body {} -- payments body
function handleGET (req, res, esClient)
{
  // Do something with the GET request
   var resMsg = '';
   console.log('Inside serer.post(getpayments)');
   console.log('req.query.uid = ' + JSON.stringify(req.query.uid));
   console.log('req.query.paymentsId = ' + JSON.stringify(req.query.paymentsId));
   var routingUid = req.query.uid;
   var paymentsId = req.query.paymentsId;
   ...
   if(paymentsId === null || paymentsId === undefined) {
    resMsg = "Error: req.query.paymentsId required to create Index in ES ->" + paymentsId;
    failure(res,resMsg,401);
   }
   ...
   esClient.ping({ requestTimeout: 30000 }, function(error)
	...
	esClient.cluster.health({},function(err,resp,status) {
	 ...
	 esClient.indices.exists(config.payments_index_name)
		 .then(function (resp) {
        //index exists
				console.log('Index ['+config.payments_index_name+'] already exists in ElasticSearch. Response is ->'+resp);
				resMsg = 'Index ['+config.payments_index_name+'] already exists in ElasticSearch'+JSON.stringify(resp);
				//check if uid exists
        ...
        esClient.get(queryBody)
          .then(function (resp) {
                //User Uid exists
                //Insert payments to the index
                var indexAliasName = routingUid+config.payments_alias_token_read;
                var columnName = config.payments_routing_column_name ;
                ...
                esClient.get(queryBody)
                .then(function (resp) {
                    resMsg = 'payments Data Retrieved Successfully!' ;
                    console.log(resMsg);
                    //esClient.close(); //use in lambda only
                    success(res,resp.hits.hits[0]);
                    },
                      function (error) {
                        resMsg = 'Error : payments document read ['+indexAliasName+'] Failed!' + JSON.stringify(error);
                        //esClient.close(); //use in lambda only
         ...
         //index dosen't exist. Create one.
    			console.log('Index does not Exists! Can not insert payments. Error value is ->'+JSON.stringify(err));
