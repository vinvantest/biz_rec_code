...
function handleGET (req, res, esClient)
{
  // Do something with the GET request
   ...
   if(coasId === null || coasId === undefined) {
    console.log("Error: req.query.coasId required to create Index in ES ->" + coasId);
    helper.failure(res,msgConfig.coas_invalid_coa_body,401);
   }
       ...
       if(error)
       {
        ...
        esClient.search(queryBodyCheckUserExists)
          .then(function (respUserCheck) {
            ...
            if(respUserCheck.hits.total === 0){
              ...
              helper.failure(res,msgConfig.coas_user_not_found+msgConfig.support_contact,500);
                ...
                .then(function (coasBody) {
                    console.log('COAs Data Retrieved Successfully!');
                    helper.success(res,coasBody);
                    },
                      function (error) {
         ...
         //index dosen't exist. Create one.
          console.log('COAs Index does not Exists! Can not get COAs data. Error value is ->'+JSON.stringify(err));
