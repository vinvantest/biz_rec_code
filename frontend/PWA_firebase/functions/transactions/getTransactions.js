...
//https://us-central1-bizrec-dev.cloudfunctions.net/getTransactionsFunction?uid=HJIOFS#53345DD&page=0&size=10
//no body {}
       ...
       esClient.search(queryBodyCheckUserExists)
         .then(function (respUserCheck) {
           ...
           if(respUserCheck.hits.total === 0){
             ...
             helper.failure(res,msgConfig.transactions_user_not_exists + msgConfig.support_contact,500);
             }
               ...
               var queryBody = {
                 ...
                 body: {
                   ...
                   size: Number(sizeVal),
                   sort: [
                        { [config.transactions_record_updated_column_name]: { order: 'desc' } }
                      ]
                  }
               };
               console.log('gettransactions search DSL -> ' + JSON.stringify(queryBody));
               esClient.search(queryBody)
               .then(function (resp) {
                   console.log('Transactions Data Retrieved Successfully!');
                   helper.success(res,resp.hits);
                   },
                     function (error) {
                       console.log('Error : Transactions document read ['+indexAliasName+'] Failed!' + JSON.stringify(error));
                       helper.failure(res,msgConfig.transactions_record_retrieve_failed,500);
                   });
               }
             else{
               ...
               helper.failure(res,msgConfig.transactions_duplicate_user_found + msgConfig.support_contact,500);
             }
         }, function (error) {
                 console.log('Error : User does not exists in database ['+config.user_index_write_alias_name+']. Contact System Adminstrator.' + error);
                 helper.failure(res,msgConfig.transactions_user_not_exists + msgConfig.support_contact,500);
        ...
        //index dosen't exist. Create one.
         console.log('Transactions Index does not Exists!. Error Value = '+JSON.stringify(err));
         helper.failure(res,msgConfig.transactions_user_index_not_exists + msgConfig.support_contact,404);
