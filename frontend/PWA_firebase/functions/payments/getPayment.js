...
function handleGET (req, res, esClient)
{
  // Do something with the GET request
   ...
   if(pymtId === null || pymtId === undefined) {
    resMsg = "Error: req.query.pymtId required to create Index in ES ->" + pymtId;
    failure(res,resMsg,401);
   }
       ...
       if(error)
       {
        ...
        esClient.search(queryBodyCheckUserExists)
          .then(function (respUserCheck) {
            ...
            if(respUserCheck.hits.total === 0){
              ...
              failure(res,resMsg,500);
                ...
                .then(function (resp) {
                    resMsg = 'payment Data Retrieved Successfully!' ;
                    console.log(resMsg);
                    success(res,resp);
                    },
                      function (error) {
                        resMsg = 'Error : payment document read ['+indexAliasName+'] Failed!' + JSON.stringify(error);
                        failure(res,resMsg,500);
                    });
                }
              else{
         ...
         //index dosen't exist. Create one.
          console.log('Index does not Exists! Can not get payments data. Error value is ->'+JSON.stringify(err));
          resMsg = 'pymtId Index does not Exists!. Error Value = '+JSON.stringify(err);
