...
//https://us-central1-bizrec-dev.cloudfunctions.net/createsuppliersFunction?uid=HJIOFS#53345DD&suppliersId=HSJ#%DFGDS#
...
function handleGET (req, res)
{
  // Do something with the GET request
   ...
   console.log('req.query.suppliersId = ' + JSON.stringify(req.query.suppliersId));
   var routingUid = req.query.uid;
   var suppliersId = req.query.suppliersId;
   ...
   if(suppliersId === null || suppliersId === undefined) {
    resMsg = "Error: req.query.suppliersId required to create Index in ES ->" + suppliersId;
    failure(res,resMsg,401);
   }
   ...
   esClient.ping({ requestTimeout: 30000 }, function(error)
	...
	esClient.cluster.health({},function(err,resp,status) {
	 ...
	 esClient.indices.exists(config.suppliers_index_name)
		 .then(function (resp) {
        //index exists
				console.log('Index ['+config.suppliers_index_name+'] already exists in ElasticSearch. Response is ->'+resp);
				resMsg = 'Index ['+config.suppliers_index_name+'] already exists in ElasticSearch'+JSON.stringify(resp);
				//check if uid exists
        ...
        esClient.get(queryBody)
          .then(function (resp) {
                //User Uid exists
                //Insert suppliers to the index
                var indexAliasName = routingUid+config.suppliers_alias_token_read;
                var columnName = config.suppliers_routing_column_name ;
         ...
         //index dosen't exist. Create one.
    			console.log('Index does not Exists! Can not insert suppliers. Error value is ->'+JSON.stringify(err));
