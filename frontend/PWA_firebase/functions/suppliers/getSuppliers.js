      ...
      if(error)
      {
       ...
       esClient.search(queryBodyCheckUserExists)
         .then(function (respUserCheck) {
           ...
           if(respUserCheck.hits.total === 0){
             ...
             helper.failure(res,msgConfig.suppliers_user_not_found + msgConfig.support_contact,500);
               ...
               .then(function (resp) {
                   console.log('Suppliers Data Retrieved Successfully!');
                   helper.success(res,resp.hits);
                   },
                     function (error) {
                       console.log('Error : Suppliers document read ['+indexAliasName+'] Failed!' + JSON.stringify(error));
        ...
        //index dosen't exist. Create one.
         console.log('Suppliers Index does not Exists!. Error Value = '+JSON.stringify(err));
