...
function handleGET (req, res, esClient)
{
  // Do something with the GET request
   ...
   if(tranId === null || tranId === undefined) {
    console.log("Error: req.query.tranId required to create Index in ES ->" + tranId);
          ...
          .then(function (respUserCheck) {
            ...
            if(respUserCheck.hits.total === 0){
              ...
              helper.failure(res,msgConfig.transactions_user_not_exists + msgConfig.support_contact,500);
                ...
                //User Uid exists
                // GET transactions Data fron transactions Index
                ...
                .then(function (respTransactionBody) {
                    console.log('Transaction Data Retrieved Successfully!');
                    helper.success(res,respTransactionBody);
                    },
                      function (error) {
                        console.log('Error : Transaction document read ['+indexAliasName+'] Failed!' + JSON.stringify(error));
                ...
                helper.failure(res,msgConfig.transactions_duplicate_records + msgConfig.support_contact,500);
                  ...
                  console.log('Error : User does not exists in database ['+config.user_index_write_alias_name+']. Contact System Adminstrator.' + error);
                  helper.failure(res,msgConfig.transactions_user_not_found + msgConfig.support_contact,500);
              });//End: check user exists
       }//end if
       else {
         //index dosen't exist. Create one.
          console.log('Transaction Index does not Exists! Can not get transactions data. Error value is ->'+JSON.stringify(err));
