...
function handleGET (req, res, esClient)
{
  // Do something with the GET request
   ...
   console.log('req.query.rulesId = ' + JSON.stringify(req.query.rulesId));
   var routingUid = req.query.uid;
   var rulesId = req.query.rulesId;
   ...
   if(rulesId === null || rulesId === undefined) {
    resMsg = "Error: req.query.rulesId required to create Index in ES ->" + rulesId;
    failure(res,resMsg,401);
   }
   ...
   esClient.ping({ requestTimeout: 30000 }, function(error)
	...
	esClient.cluster.health({},function(err,resp,status) {
	 ...
	 esClient.indices.exists(config.rules_index_name)
		 .then(function (resp) {
        //index exists
				console.log('Index ['+config.rules_index_name+'] already exists in ElasticSearch. Response is ->'+resp);
				resMsg = 'Index ['+config.rules_index_name+'] already exists in ElasticSearch'+JSON.stringify(resp);
				//check if uid exists
        ...
        esClient.get(queryBody)
          .then(function (resp) {
                //User Uid exists
                //Insert rules to the index
                var indexAliasName = routingUid+config.rules_alias_token_read;
                var columnName = config.rules_routing_column_name;
         ...
         //index dosen't exist. Create one.
    			console.log('Index does not Exists! Can not insert rules. Error value is ->'+JSON.stringify(err));
