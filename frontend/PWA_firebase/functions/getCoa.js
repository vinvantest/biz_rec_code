...
//https://us-central1-bizrec-dev.cloudfunctions.net/createcoasFunction?uid=HJIOFS#53345DD
//no body {} -- coas body
function handleGET (req, res, esClient)
{
  // Do something with the GET request
   ...
   console.log('req.query.coasId = ' + JSON.stringify(req.query.coasId));
   ...
   esClient.ping({ requestTimeout: 30000 }, function(error)
	...
	esClient.cluster.health({},function(err,resp,status) {
	 ...
	 esClient.indices.exists(config.coas_index_name)
		 .then(function (resp) {
        //index exists
				console.log('Index ['+config.coas_index_name+'] already exists in ElasticSearch. Response is ->'+resp);
				resMsg = 'Index ['+config.coas_index_name+'] already exists in ElasticSearch'+JSON.stringify(resp);
				//check if uid exists
        ...
        esClient.get(queryBody)
          .then(function (resp) {
                //User Uid exists
                //Insert coas to the index
                var indexAliasName = routingUid+config.coas_alias_token_read;
                var columnName = config.coas_routing_column_name ;
         ...
         //index dosen't exist. Create one.
    			console.log('Index does not Exists! Can not insert coas. Error value is ->'+JSON.stringify(err));
